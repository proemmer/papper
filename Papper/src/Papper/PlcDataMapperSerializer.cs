...
namespace Papper
{
    public class PlcDataMapperSerializer
    {
        ...
        {
            if (type == null) return ExceptionThrowHelper.ThrowArgumentNullException<byte[]>(nameof(type));
            if (data == null) return ExceptionThrowHelper.ThrowArgumentNullException<byte[]>(nameof(data));
        ...
        {
            if (type == null) return ExceptionThrowHelper.ThrowArgumentNullException<object>(nameof(type));
            if (data == null) return ExceptionThrowHelper.ThrowArgumentNullException<object>(nameof(data));
        ...
        {
            if (_mappingEntryProvider == null) return 0;
            if (type == null) return ExceptionThrowHelper.ThrowArgumentNullException<int>(nameof(type));
            var entry = _mappingEntryProvider.GetMappingEntryForType(type!);
            if (entry == null) ExceptionThrowHelper.ThrowMappingAttributeNotFoundForTypeException(type!);
            return entry!.PlcObject.ByteSize;
        }
